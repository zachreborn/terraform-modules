...
| <a name="input_dr_cold_storage_after"></a> [dr\_cold\_storage\_after](#input\_dr\_cold\_storage\_after) | (Optional) The number of days after creation that a recovery point is moved to cold storage. Backups transitioned to cold storage must remain in cold storage for at least 90 days. | `number` | `null` | no |
