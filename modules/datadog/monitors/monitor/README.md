...
- **`required_version >= 1.3.0`**: This module uses the two-argument form of `optional()` (e.g. `optional(string, "published")`, `optional(bool, false)`) for object-attribute defaults, which was introduced in Terraform 1.3.0 / OpenTofu 1.6.0. This is stricter than the default `>= 1.0.0` used in AWS modules and matches the other Datadog modules in this library.
