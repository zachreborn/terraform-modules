...
| <a name="input_webhook_header_values"></a> [webhook\_header\_values](#input\_webhook\_header\_values) | Map of header values that contain a credential (e.g. an Authorization header), keyed by the<br/>webhook's logical name (matching a key in var.webhooks) and then by header name. Populate an<br/>entry here -- and omit 'value' on the corresponding header entry in var.webhooks -- for any<br/>header whose value should not appear in Terraform/OpenTofu plan output. The provider's webhook<br/>header schema has no 'sensitive' flag of its own (unlike agent\_pool's), so var.webhooks itself<br/>cannot mark individual header values sensitive; routing the value through this separate,<br/>always-sensitive variable is the only way to keep it out of plan output.<br/><br/>Example:<br/>  webhook\_header\_values = {<br/>    run\_notifications = {<br/>      Authorization = "Bearer my-secret-token"<br/>    }<br/>  } | `map(map(string))` | `{}` | no |
