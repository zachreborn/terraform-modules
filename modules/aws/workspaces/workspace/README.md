...
| <a name="input_enable_default_kms_key"></a> [enable\_default\_kms\_key](#input\_enable\_default\_kms\_key) | (Optional) If true (the default) and an entry in var.workspaces omits volume\_encryption\_key, this module creates one shared AWS KMS customer-managed key (via modules/aws/kms) per distinct Region among those entries (see the region field), and uses that Region's key ARN as the entry's volume\_encryption\_key -- AWS requires the key to be in the same Region as the desktop, so entries in different Regions never share a key. The keys' aliases are derived from kms\_key\_alias\_prefix -- see that variable for the exact naming behavior. Set to false to require every entry to supply its own volume\_encryption\_key, or to rely on the AWS-managed alias/aws/workspaces key by leaving volume\_encryption\_key null. | `bool` | `true` | no |
| <a name="input_kms_key_alias_prefix"></a> [kms\_key\_alias\_prefix](#input\_kms\_key\_alias\_prefix) | (Optional) Passed as name\_prefix to modules/aws/kms for every shared default KMS key created when enable\_default\_kms\_key is true and at least one entry needs it (one per distinct Region -- see enable\_default\_kms\_key). Ignored otherwise. modules/aws/kms creates the alias as aws\_kms\_alias with this name\_prefix, so the actual alias is alias/<kms\_key\_alias\_prefix><randomly-generated suffix>, not an exact alias/<kms\_key\_alias\_prefix> name; the same prefix is reused in every Region since aliases are Region-scoped and cannot collide across Regions. | `string` | `"workspaces"` | no |
...
| <a name="output_kms_key_arn"></a> [kms\_key\_arn](#output\_kms\_key\_arn) | Map of shared default KMS key ARNs created for volume encryption, keyed by Region. One key is created per distinct Region among entries that need it (see var.workspaces' region field); empty when enable\_default\_kms\_key is false or no entry needed one. |
