...
| <a name="input_domain_names"></a> [domain\_names](#input\_domain\_names) | Map of domain names to create for the API Gateway | <pre>map(object({<br/>    domain_name_configuration = object({<br/>      certificate_arn                        = string<br/>      endpoint_type                          = optional(string, "REGIONAL")<br/>      ownership_verification_certificate_arn = optional(string)<br/>      security_policy                        = optional(string, "TLS_1_2")<br/>    })<br/>  }))</pre> | `{}` | no |
