...
| <a name="input_tags"></a> [tags](#input\_tags) | A map of tags to assign to the API. | <pre>map(object({<br/>    name        = required(string)<br/>    description = optional(string)<br/>    target_arns = optional(list(string))<br/>  }))</pre> | `{}` | no |
