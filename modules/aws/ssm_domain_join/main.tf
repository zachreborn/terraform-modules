    ...
    mainSteps = [
      {
        ...
        inputs = {
          runCommand = [
            ...
            "$_filter = @(@{Name='resource-id';Values=$_iid}, @{Name='key';Values='Name'})",
            "Import-Module AWS.Tools.EC2 -ErrorAction SilentlyContinue",
            "try { $_tagVal = (Get-EC2Tag -Region $_rgn -Filter $_filter -ErrorAction Stop | Select-Object -First 1).Value } catch { Write-Log ('EC2 tag lookup failed: ' + $_.Exception.Message); exit 1 }",
            "if ($_tagVal) { $_base = $_tagVal } else { Write-Log 'Name tag not set, using current hostname'; $_base = $env:COMPUTERNAME }",
