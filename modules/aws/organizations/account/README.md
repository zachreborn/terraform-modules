...
spaces, and the characters `+ - = . _ : / @`. Both the `tags` variable and each `accounts[key].tags`
entry are validated against this set at `tofu plan`/`validate` time (rather than failing mid-`apply`):
tag keys must be non-empty, while tag values may be empty.
