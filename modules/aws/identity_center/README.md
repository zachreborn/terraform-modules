...
As part of adding the `permission_sets` composition above, the `permission_set` submodule's `assignment_ids` output is now keyed by `"<group_name>_<account_id>"` (the same key already used by its underlying `for_each`) instead of `"<principal_id>_<account_id>"` (parsed from the assignment resource's own runtime `id`). The old derivation could produce a duplicate-map-key error in the (rare) case of two assignments resolving to the same parsed key, and made native `tofu test` coverage of multiple assignments impossible, since mocked resource attributes are not unique per instance. If you index `permission_set`'s `assignment_ids` output by the old `<principal_id>_<account_id>` shape, update those references to the new `<group_name>_<account_id>` key before upgrading -- and see the note above, since that key now moves again to `<group_name>_<label>`.
