...
| <a name="input_encrypted"></a> [encrypted](#input\_encrypted) | (Optional) Whether to encrypt the root EBS volume. Defaults to false. The Zscaler Marketplace AMI snapshot is unencrypted; overriding to encrypted=true at launch breaks the product code association and causes UnsupportedOperation. To encrypt, supply a kms\_key\_id and use post-launch re-encryption. | `bool` | `false` | no |
